...
// !!! ВОЗВРАЩАЕМ searchAnime из shikimoriApi !!!
...
const HomePage = () => {
    ...
    // --- Функции ---
    // !!! fetchData снова использует searchAnime !!!
    ...
    const loadGenresForButtons = useCallback(async () => { setIsLoadingGenres(true); setErrorGenres(null); try { const { data: genreData, error: genreError } = await getGenres(); if (genreError) throw genreError; setGenres((genreData || []).slice(0, 5)); } catch (err) { setErrorGenres(err); console.error("Error loading genres:", err); } finally { setIsLoadingGenres(false); } }, []);
