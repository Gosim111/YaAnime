...
const HomePage = () => {
    ...
    // ... остальные состояния ...
    const [popularAnime, setPopularAnime] = useState([]); const [seasonalAnime, setSeasonalAnime] = useState([]); const [topRatedAnime, setTopRatedAnime] = useState([]); const [isLoadingPopular, setIsLoadingPopular] = useState(true); const [isLoadingSeasonal, setIsLoadingSeasonal] = useState(false); const [isLoadingTopRated, setIsLoadingTopRated] = useState(false); const [errorPopular, setErrorPopular] = useState(null); const [errorSeasonal, setErrorSeasonal] = useState(null); const [errorTopRated, setErrorTopRated] = useState(null); const [seasonalLoaded, setSeasonalLoaded] = useState(false); const [topRatedLoaded, setTopRatedLoaded] = useState(false); const [searchTerm, setSearchTerm] = useState(''); const [showScrollTop, setShowScrollTop] = useState(false);
    ...
    const loadGenresForButtons = useCallback(async () => { setIsLoadingGenres(true); setErrorGenres(null); try { const { data: genreData, error: genreError } = await getGenres(); if (genreError) throw genreError; setGenres((genreData || []).slice(0, 5)); } catch (err) { setErrorGenres(err); console.error("Error loading genres:", err); setGenres([]); } finally { setIsLoadingGenres(false); } }, []); // Устанавливаем пустой массив при ошибке
    ...
    // Рендеринг
    return ( <div className={styles.homePage}> <section className={styles.heroSection}> <video ref={videoRef} className={styles.heroVideoBackground} autoPlay muted loop playsInline poster="/hero-background.jpg"> <source src="/hero-video.mp4" type="video/mp4" /> Ваш браузер не поддерживает видео. </video> <div className={styles.heroOverlay}></div> <div className={styles.heroContentWrapper}> <div className={styles.heroContent}> <h1 className={`${styles.heroTitle} ${styles.fadeInUp}`}>Твой компас в мире аниме</h1> <p className={`${styles.heroSubtitle} ${styles.fadeInUpDelay1}`}> Находи, смотри и обсуждай любимые тайтлы. </p> <form className={`${styles.searchForm} ${styles.fadeInUpDelay2}`} onSubmit={handleSearch}> <input type="search" placeholder="Найти аниме..." className={styles.searchInput} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /> <button type="submit" className={styles.searchButton} aria-label="Поиск"><SearchIcon /></button> </form> {/* Проверка isLoadingGenres И errorGenres */} {isLoadingGenres ? ( <div className={styles.heroGenreLoading}>Загрузка жанров...</div> ) : errorGenres ? ( <div className={styles.heroGenreError}>{errorGenres.message || 'Ошибка загрузки жанров.'}</div> ) : Array.isArray(genres) && genres.length > 0 ? ( <div className={`${styles.heroGenreButtons} ${styles.fadeInUpDelay3}`}> <span>Жанры:</span> {genres.map(genre => ( <Link key={genre.id} to={`/catalog?genre=${genre.id}`} className={styles.heroGenreButton}>{genre.russian || genre.name}</Link> ))} </div> ) : null /* Ничего не рендерим, если жанров нет и ошибки нет */ } </div> </div> </section> <div className={styles.mainContent}> <AnimeSection key="popular" title="Популярное сейчас" icon={StarIcon} iconColor={accentColor} animeList={popularAnime} isLoading={isLoadingPopular} error={errorPopular} layout="carousel" usePagination={true} linkToCatalog="/catalog?order=popularity" onVisible={() => {}} /> <div className={styles.sectionDivider}></div> <div ref={seasonalSectionRef}> <AnimeSection key="seasonal" title="Онгоинги Сезона" icon={CalendarDaysIcon} iconColor={accentColor} animeList={seasonalAnime} isLoading={isLoadingSeasonal} error={errorSeasonal} layout="grid" gridClass={styles.gridSevenCols} linkToCatalog={`/catalog?status=ongoing&order=popularity`} onVisible={loadSeasonal} /> </div> <div className={styles.sectionDivider}></div> <div ref={topRatedSectionRef}> <AnimeSection key="top-rated" title="Топ Рейтинга" icon={TrendingUpIcon} iconColor={accentColor} animeList={topRatedAnime} isLoading={isLoadingTopRated} error={errorTopRated} layout="grid" gridClass={styles.gridSevenCols} linkToCatalog="/catalog?order=ranked" onVisible={loadTopRated} /> </div> </div> {showScrollTop && ( <button onClick={scrollTop} className={`${styles.scrollTopButton} ${showScrollTop ? styles.show : ''}`} title="Наверх"> ↑ </button> )} </div> );
};
