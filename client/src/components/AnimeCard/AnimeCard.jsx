...
const SHIKIMORI_URL_PREFIX = 'https://shikimori.one'; // Используется для постеров

// --- Вспомогательные компоненты (снова используем поля из Shikimori) ---
...
const AnimeCard = ({ anime }) => {
    // !!! ПРОВЕРКА ДАННЫХ из Shikimori API /animes !!!
    if (!anime || !anime.id || !(anime.russian || anime.name)) { // Проверяем ID и название
    ...
    const linkTo = `/anime/${anime.id}`; // Ссылка использует Shikimori ID
    const displayTitle = anime.russian || anime.name; // Приоритет русскому названию
    const fullTitleForHint = anime.name || anime.russian; // Оригинальное для подсказки

    // Извлечение данных из объекта Shikimori
    ...
    return (
        // Используем Link, так как ID всегда есть (проверили выше)
        <Link to={linkTo} className={styles.cardLink} title={fullTitleForHint}>
            <article className={styles.card}>
                ...
                <div className={styles.info}>
                    <div className={styles.titleSeasonWrapper}>
